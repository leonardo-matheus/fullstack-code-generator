<template>
  <q-btn
    ...
    unelevated
    dense
    :class="`animated zoomIn lv-btn ${classTheme}`"
    :loading="loading"
    :round="round"
    :href="href"
    :size="size ? size : 'md'"
  ...
  >
    <q-icon
      v-if="icon"
      :name="icon"
      style="font-size: 1em !important; opacity: 0.7"
    />
    <span
      v-if="label"
      :class="labelVisibility ? 'gt-xs' : ''"
      :style="`${icon ? 'padding-left:5px; ' : ''}`"
      >{{ label }}</span
    >
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const classTheme = computed(() => {
      ...
      if (props.color)
        isColorBase = props.color.indexOf("-") > -1 ? true : false;
