<template>
  <div class="col-12 row header-page">
    <div
      :class="`header-page-left row text-primary ${
        split && !single ? 'col-6' : 'col-12'
      } ${single ? '' : 'col-sm-5'}`"
    >
      <!-- Back Button -->
      <template v-if="showBack">
        <lv-btn
          round
          class="back-btn bg-grey-2 no-border"
          size="xs"
          v-close-popup
    ...
    <div
      :class="`header-page-right row q-gutter-sm ${
        split && !single ? 'col-6' : 'col-12'
      } ${single ? '' : 'col-sm-7'}`"
    >
...
export default defineComponent({
  ...
  props: {
    ...
    split: {
      // split area left & right always set col 6
      type: Boolean,
      default: false,
    },
    single: {
      // split area will not effected, becase area will be col-12 as default
      type: Boolean,
      default: false,
    },
  },
