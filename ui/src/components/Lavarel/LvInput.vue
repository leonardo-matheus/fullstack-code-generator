  ...
  <div :class="`lv-field col-${col}`">
    ...
    >
      <template v-slot:control="{ id, floatingLabel, modelValue, emitValue }">
        ...
        <template
          v-else-if="mode === 'date' || mode === 'datetime' || mode === 'time'"
        >
          <template v-if="!isReadOnly">
            ...
            >
              <q-date
                ...
                :options="optionDate"
                @update:model-value="
                  (val) => handlerInputDate(refDate, refTime, val, emitValue)
                "
              >
              </q-date>
            </q-popup-proxy>
            ...
            >
              ...
              >
                <div class="row items-center justify-end">
                  <q-btn v-close-popup label="Oke" color="primary" flat />
                </div>
              </q-time>
            </q-popup-proxy>
          </template>
          ...
          <div
            v-else
            class="self-center full-width no-outline q-field__input"
            tabindex="0"
          >
            {{
              inputValue
          ...
          <template v-if="!isReadOnly">
            ...
            >
              <q-date
                ...
                :options="optionDate"
                @update:model-value="
                  (val) => handlerInputDate(refDate, null, val, emitValue)
                "
              >
                <div class="row items-center justify-end">
                  <small class="text-grey-7"
                    >Double click on same day to set one day range</small
                  >
                  <q-btn
                    v-if="inputValue"
                    v-close-popup
...
export default defineComponent({
  ...
  props: {
    ...
    // spec
    mode: {
      // enum( text, currency, number, date, datetime, time, daterange )
      type: String,
      default: "text",
    },
    decimal: {
  ...
  setup(props, { emit }) {
    ...
    onBeforeMount(() => {
      if (props.modelValue) {
        if (props.mode === "number" || props.mode === "currency")
          firstInitDataNumber(props.modelValue);
    ...
    watch(inputValue, () => {
      if (props.type === "number")
        inputValue.value = Helper.toNumber(inputValue.value);
    ...
    function handlerInputDate(refDate, refTime, val, emitValueEvt) {
      ...
      if (refTime && refT.value && props.mode === "datetime")
        refT?.value?.show();
    }
