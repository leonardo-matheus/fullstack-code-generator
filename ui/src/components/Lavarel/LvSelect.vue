<template>
  <div :class="`lv-field col-${col} row`">
    ...
    <q-select
      :ref="refs[refName]"
      :label-slot="useInnerLabel"
      :outlined="outlined"
      :borderless="borderless"
      dense
      :class="`col-12 ${elClass}`"
      v-model="inputValue"
    ...
    >
      ...
      <template v-slot:selected-item="scope">
        <div
          class="lv-select_display_value ellipsis"
      ...
      <template v-slot:option="scope">
        <q-item v-bind="scope.itemProps" class="ph">
          <q-item-section>
            <q-item-label
              ><span v-html="optLabel(scope.opt)"></span
            ></q-item-label>
          </q-item-section>
        </q-item>
        <q-separator />
...
export default defineComponent({
  ...
  props: {
    ...
    url: {
      // only path url, if need add query params use "url-params"
      type: String,
      default: null,
    },
    urlParams: {
    ...
    searchKey: {
      // it will be query params search ex: ?_contains=
      type: String,
      default: "_contains",
    },
    limit: {
      // it will be query params search ex: ?limit=8
      type: [String, Number],
      default: "8",
    },
    searchBy: {
      // the value is avail column of modules
      type: Array,
      default: () => {
        return ["name"];
      },
  ...
  setup(props, { emit }) {
    ...
    const searchField = computed(() => {
      let col = "name";
      if (props.optionLabel && typeof props.optionLabel !== "function")
        col = props.optionLabel;
      return col;
    });
    ...
    const fixRules = computed(() => {
      ...
      if (props.required && props.multiple)
        res = [(val) => (!!val && val.length > 0) || "Field is required"];
      return res;
    });
    ...
    function emiters(val) {
      const selected =
        select.tmp.find(
          (r) => r[optionValueDefault.value] === inputValue.value
        ) || null;
    ...
    function handleDefaultData() {
      if (props.url) {
        ...
        if (props.defaultData) {
          ...
          // fetch to server if default data not defined
          if (val) {
            if (props.multiple) {
    ...
    async function fetchLocal(val, update) {
      ...
      if (val) {
        const res = await Handler.filterSelect(
          val,
          update,
          "select",
          options,
    ...
    function initOptions(list, withTmp = true) {
      if (
        ...
        !list.find(
          (r) =>
            r[optionValueDefault.value] ===
            props.defaultData[optionValueDefault.value]
        )
      ) {
    ...
    function optLabel(val) {
      var res =
        props.optionLabel instanceof Function
          ? props.optionLabel(val)
          : val[props.optionLabel || searchField.value];
      return res;
    }
    ...
    function onFocus() {
      ...
      if (ref) {
        // handle when click focus element
        ref.focus();
      }
