<template>
  <q-table
    ...
    @row-dblclick="
      (evt, row, index) => rowClickHandler(evt, row, index, 'dblclick')
    "
    @row-contextmenu="
      (evt, row, index) => rowClickHandler(evt, row, index, 'contextmenu')
    "
  >
    <template v-slot:top v-if="!hideTop">
      <template v-if="table.selected.length">
        <lv-btn
          ...
          v-if="actionBtn"
          :label="`${table.isTrashed ? 'Re-Activate' : 'Delete'} (${
      ...
      <template v-else>
        ...
        <q-toggle
          ...
          size="sm"
          label="Trash"
          v-model="table.isTrashed"
          @click="onRefresh"
          class="q-pl-sm animated fadeIn label-toggle-sm"
        />
        ...
        <q-input
          ref="focusSearch"
          debounce="500"
          :placeholder="`Search by ${table.searchBy.join(',')}...`"
          class="input-search-bar clearable no-shadow"
        ...
        >
          <template v-slot:append>
            ...
            <small
              v-if="!noShorthand"
              style="font-size: 13px; font-weight: 600; color: #a6a5a5"
              >(F1)</small
            >
          </template>
      ...
      <q-tr v-if="table" :props="props">
        <q-th v-if="table.action" style="padding: 0px !important">
          <q-checkbox
            ...
            @update:model-value="(val) => onSelectedAll(val)"
            v-model="checkAll"
          />
        </q-th>
    ...
    <template v-slot:pagination>
      <span v-if="table"
        >1 - {{ table.data.length }} of {{ table.pagination.rowsNumber }}</span
      >
    </template>
  </q-table>
</template>
...
export default defineComponent({
  ...
  props: {
    ...
    config: {
      // table object structure, use : Handler.table() for standard structure
      type: Object,
      default: null,
    },
    height: {
    ...
    urlPath: {
      // custom endpoint path, need for fetch, delete & restore
      type: String,
      default: null,
    },
    /* USAGE :
    ...
    */
    urlParams: {
      // custom parameter for url queries
      type: [String, Array],
      default: null,
    },
    /* USAGE :
    ...
    */
    ...
    searchBy: {
      // fill with column name on table module, ex : code, remarks
      type: Array,
      default: () => {
        return [];
      },
  ...
  setup(props, { emit }) {
    ...
    function onRefresh() {
      if (props.urlPath) {
        setTimeout(() => {
          // delay for wait if parent has change some props
          getDataByUrl(true);
        }, 100);
      }
    }
    ...
    function compileSearchQuery(table) {
      ...
      if (table.searchBy && table.searchBy.length) {
        if (table.search)
          endpoint += `${endpoint}&_contains=${table.searchBy.join(",")}:${
            table.search
          }`;
      }
    ...
    function prepareFetch() {
      ...
      const perpage =
        rowsPerPage === 0 ? table.value.pagination.rowsNumber : rowsPerPage;
      ...
      if (table.value.sort)
        endpoint =
          endpoint +
          `&_order=${table.value.sort.name}:${table.value.sort.type}`;
    ...
    function setData(data, reformat = false) {
      if (reformat) {
        ...
        if (data.data.length > 0 && data.to <= data.total) {
          ...
          if (table.value.data.length === data.total)
            table.value.pagination.isLast = true;
    ...
    function tableShowSearch(col) {
      if (table.value.searchColumn) {
        ...
        if (f >= 0) {
          ...
          if (column.searchable && column.name !== "action")
            table.value.columns[f].searching.show = !column.searching.show;
        }
      }
    }
