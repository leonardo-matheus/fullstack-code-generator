...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    function initColumns() {
      if (table.value.visibleColumns && table.value.visibleColumns.length)
        columns.value = table.value.visibleColumns;
      else columns.value = columnList.value;
    }

    function initSearchBy() {
      if (table.value.searchBy && table.value.searchBy.length)
        searchBy.value = table.value.searchBy;
    ...
    function saveSetting() {
      ...
      table.value = {
        ...Handler.table(
          table.value.columns,
          table.value.limit,
          table.value.cacheVisibilityColumn
        ),
      };
