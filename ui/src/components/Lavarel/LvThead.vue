<template v-if="column">
  <div>
    <template
      v-if="column.searchable === true || column.searchable === 'advance'"
    >
      <q-icon
        name="filter_list"
        :class="`${
          column.searching.show ? 'text-primary column-filtered' : ''
        } link`"
      >
        <q-popup-proxy>
          <div class="q-pa-md row filter-thead bg-white" style="width: 360px">
                ...
                >
                  <q-menu transition-show="jump-down" transition-hide="jump-up">
                    <q-list style="min-width: 100px">
                      ...
                      <q-item
                        clickable
                        @click="updateColumnType('datetime', 2)"
                      >
                        <q-item-section>Date Time</q-item-section>
                      </q-item>
                      <q-separator />
                      ...
                      <q-item
                        clickable
                        @click="updateColumnType('currency', 2)"
                      >
                        <q-item-section>Currency</q-item-section>
                      </q-item>
                    </q-list>
            ...
            <div class="col-12 text-right row">
              ...
              >
                <q-tooltip
                  >Will searching automated after typing or change some
                  configuration</q-tooltip
                >
              </lv-toggle>
    ...
    <template v-else-if="column.searchable">
      <q-input
        v-if="column.searching.show"
        dense
        outlined
        :debounce="400"
        v-model="search"
        class="search-table-head animated fadeIn"
        :placeholder="`Search by ${label}...`"
        :type="column.searchable"
      ...
      >
        <template v-slot:append>
          ...
          <q-icon
            ...
            @click="closeSearch(table.columns.indexOf(column))"
          />
        </template>
      </q-input>
      <template v-else>
        <q-icon
          name="search"
          class="link"
          @click="showSearch(table.columns.indexOf(column))"
        >
    ...
    <template v-if="column.sort && !column.searching.show">
      <template v-if="table.sortColumn">
        <q-icon
          ...
          name="arrow_upward"
          class="link hover-primary"
          @click="sortData(column)"
        >
          <q-tooltip
            >Click to sort:
            {{ column.sort == "asc" ? "desc" : "asc" }}</q-tooltip
          >
        </q-icon>
        <q-icon
