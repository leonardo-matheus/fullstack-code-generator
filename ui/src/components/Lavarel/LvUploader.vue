<template>
  <div class="lv-field">
    ...
    <q-avatar
      ...
      :size="size"
      :round="rouded"
      class="lv-image-uploader bordered"
    >
    ...
    >
      ...
      <template v-slot:control>
        <small
          class="self-center full-width no-outline text-grey-7"
          tabindex="0"
          >{{ fileDisplay }}</small
        >
      </template>
      ...
      <template v-slot:hint
        ><span class="text-red-9"> {{ error }} </span></template
      >
...
export default defineComponent({
  ...
  props: {
    ...
    external: {
      // flag for upload to external url
      type: Boolean,
      default: false,
    },
    autoUpload: {
      // flag for trigger upload after file picked
      type: Boolean,
      default: false,
    },
  },
  setup(props, { emit }) {
    ...
    const onFileSelected = (event) => {
      ...
      if (
        ...
        !props.extension.includes(file.type.split("/").pop())
      ) {
      ...
      reader.onload = () => {
        ...
        emit("picked", {
          name: fileDisplay.value,
          blob: inputValue.value,
          raw: rawFile.value,
        });
