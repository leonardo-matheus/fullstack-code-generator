<template>
  <q-chip color="primary" :class="`chip-profile text-white `">
    ...
    <div class="chip-text">
      <span style="text-transform: none">{{ user.username }}</span>
    </div>
    <q-menu
      @before-show="onRefresh"
    ...
    >
      <div class="no-wrap">
        <div :class="`text-center pt-1 bg-primary`">
          <div
            class="q-mb-md text-center text-white q-py-md link"
          ...
          >
            ...
            <br />
            <i class="animated fadeIn">{{ user.name }}</i
            ><br />
            <span class="animated fadeIn">{{ user.email }}</span> <br />
            <small class="text-yellow"
              >{{ user.role ? user.role.name : "role-name" }} ({{
                user.role ? user.role.slug : "role-slug"
              }})</small
            ><br />
          </div>
        </div>
        ...
        <div class="q-px-md bg-light q-pb-md">
          <q-toggle
            size="sm"
            @update:model-value="(val) => changePrimaryDrawer(val)"
          ...
          <q-toggle
            size="sm"
            @update:model-value="(val) => changeMenuMode(val)"
            v-model="topMenuMode"
            left-label
            label="Top Menu Mode"
            class="mb-1"
          ...
          <q-toggle
            v-if="!topMenuMode"
            size="sm"
            @update:model-value="(val) => changeMiniMode(val)"
            v-model="miniMenuMode"
            left-label
            label="Mini Mode Menu"
            class="mb-1"
          />
          <br v-if="!topMenuMode" />
          <q-toggle
            size="sm"
            @update:model-value="(val) => switchAction(val)"
            v-model="actionModal"
            left-label
            label="Action on Modal"
            class="mb-1"
          />
          <br />
          <q-btn-dropdown
            flat
            class="q-mb-sm full-width bg-grey-2 text-capitalize q-mt-sm"
            color="primary"
            label="More..."
          >
            <q-list>
              ...
              <q-item
                ...
                clickable
                v-close-popup
                @click="openLink('change-password')"
              >
                <q-item-section avatar>
                  <q-avatar
                    icon="gpp_good"
                ...
                <q-item-section
                  ><q-item-label>Change Password</q-item-label></q-item-section
                >
              </q-item>
              ...
              <q-item dense clickable v-close-popup @click="reloadPermission">
                ...
                <q-item-section
                  ><q-item-label
                    >Reload Permissions</q-item-label
                  ></q-item-section
                >
              </q-item>
            </q-list>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const { primaryDrawer, actionModal, miniMenuMode, topMenuMode, userInfo } =
