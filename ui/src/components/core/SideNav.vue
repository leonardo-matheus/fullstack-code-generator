<template>
  <q-drawer
    ...
    show-if-above
    v-model="drawer"
    :class="`side-menu ${primaryDrawer ? 'primary' : ''}`"
    :width="260"
    :breakpoint="500"
    :mini="isMiniMode"
  ...
  >
    <q-img class="absolute-top head-menu" :style="`height: ${headSize}`">
      ...
      <div class="logo bg-transparent full-width" v-if="!isMiniMode">
        <img
          :src="primaryDrawer ? $Config.appLogoLight() : $Config.appLogo()"
          width="82"
          class="text-dark text-bold text-h4"
          alt="Lavarel"
        />
        <br />
        ...
        <q-btn
          ...
          dense
          size="sm"
          class="apiroot-label text-normal"
          :label="$Config.getApiRoot()"
      ...
      <div class="logo bg-transparent" v-if="isMiniMode">
        ...
        <br />
        <q-btn
          @click="disableMiniMode"
          style="position: relative; top: 12px"
          flat
          dense
          size="sm"
          class="disable-mini text-normal"
    ...
    <q-scroll-area
      :style="`height: calc(94% - ${headSize}); margin-top: ${headSize};`"
      :thumb-style="$Config.scrollThumbStyle()"
      :bar-style="$Config.scrollBarStyle()"
    >
...
export default defineComponent({
  ...
  setup() {
    ...
    const isTopNav = computed(() => {
      return GlobalStore.getTopMenuMode;
    });
    const isMiniMode = computed(() => {
      return GlobalStore.getMiniMenuMode;
    });
    const primaryDrawer = computed(() => {
      return GlobalStore.getPrimaryDrawer;
    });
    ...
    watch(routeName, async (newVal, val) => {
      formatedMenus.value = filterPermissionMenu(
        formatDrawer(formatedMenus.value)
      );
    });
      ...
      menus.map((r) => {
        data = [
          ...data,
          {
            ...
            collapse: r.children
              ? !!r.children.find((child) => child.route === routeName.value)
              : false,
          },
        ];
      });
    ...
    function isRouteNameMatch(currentRoute, route) {
      return (
        route.route === currentRoute || currentRoute.includes(`${route.route}-`)
      );
    }
    ...
    function formatDrawer(menus) {
      ...
      menus.map((r) => {
        ...
        const children = hasChildren
          ? formatDrawer(r.children, currentRoute)
          : [];
        ...
        data = [
          ...data,
          {
            ...r,
            collapse:
              isCurrentRoute || children.some((child) => child.collapse),
            children,
          },
        ];
      });
    ...
    function filterPermissionMenu(arr) {
      ...
      return arr
        .map((item) => {
          ...
          return {
            ...item,
            children: filterPermissionMenu(item.children).filter((child) =>
              Handler.checkPermissionMenu(child.permissions, permissionList)
            ),
          };
        })
        .filter((item) =>
          Handler.checkPermissionMenu(item.permissions, permissionList)
        );
    }
