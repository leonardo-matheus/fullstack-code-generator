<template>
  <q-list
    :class="`deepth${deepthMenu ? '-' + deepthMenu : ''}`"
    :style="paddingSide"
  >
    <template v-for="(menu, index) in formatedMenus" :key="index">
      ...
      <div v-else class="side-menu-wrapper">
        <q-item
          ...
          clickable
          @click="
            formatedMenus[index].collapse = !formatedMenus[index].collapse
          "
          :class="[
            'side-menu-item',
        ...
        >
          ...
          <q-item-section side>
            <q-item-label caption
              ><q-icon :name="menu.collapse ? 'expand_less' : 'expand_more'"
            /></q-item-label>
          </q-item-section>
          <q-tooltip
            v-if="miniMode"
      ...
      <q-slide-transition>
        <side-nav-list
          v-if="menu.collapse && menu.children.length"
          :menus="menu.children"
          :miniMode="miniMode"
          :deepth="deepthMenu"
          class="sub-menu"
