  ...
  <q-header bordered v-if="isTopNav">
    <q-bar style="height: 48px">
      ...
      <q-btn dense flat @click="showSubMenu = !showSubMenu">
        <q-icon
          :name="showSubMenu ? 'keyboard_arrow_up' : 'keyboard_arrow_down'"
    ...
    <div
      v-if="topMenu.length && showSubMenu"
      class="submenu-container row items-center"
    >
      <div
        v-for="(row, index) in topMenu"
        v-bind:key="index"
        class="submenu-list animated fadeIn"
      >
        <span
          v-html="row.name"
          @click="$Handler.topMenuCallEvent(topMenu, index)"
          :class="`${row.class ? row.class : ''} submenu-root`"
        ></span>
        <q-menu
          v-if="$Handler.topMenuHasSub(row)"
