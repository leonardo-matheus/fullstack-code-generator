  ...
  <div class="row bg-white">
    ...
    <lv-header-page class="bg-white" split>
      <template v-slot:left>
        <div
          class="col-12 col-sm-6 col-md-4 page-title animated zoomIn q-pt-xs"
        >
          Notifications
        </div>
        <lv-toggle
          ...
          v-model="unRead"
          label="Unread"
          @input="onChangeUnread"
        />
        <div class="col-12 text-indigo-9">
          <small>*Double click row for see detail notice</small>
        </div>
      </template>
      <template v-slot:right>
        <template v-if="table.selected.length && unRead">
          <lv-btn
            soft
            icon="check"
            :label="`Mark read selected (${table.selected.length})`"
            labelVisibility
            @click="makeRead"
          />
        </template>
    ...
    <q-dialog
      v-model="modalDetail.show"
      @hide="modalDetail.show = false"
      persistent
    >
      <q-card>
        <q-bar class="bg-primary text-white">
        ...
        <q-card-section class="q-pa-sm" v-if="modalDetail.data">
          ...
          <div class="col-12 text-bold q-pb-sm">
            <q-chip
              ...
              text-color="white"
              :color="`${
                modalDetail.data.type === 'direct' ? 'teal' : 'orange-8'
              }`"
            ...
            <q-chip
              v-if="modalDetail.data.category"
              size="sm"
              square
              color="primary"
              text-color="white"
      ...
      ...Handler.table([
        {
          ...
          width: "180px",
          searchable: "date",
          format: (val) => (val ? Helper.beautyDate(val, " ", true) : ""),
        },
        {
          name: "creator",
          label: "From",
