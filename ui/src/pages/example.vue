<template>
  <div class="row">
    <div class="q-pa-md col-12 text-bold text-primary text-h4 q-mb-md">
      Example Components
    </div>
    ...
    <lv-container id="input" class="col-12" height="850px">
      ...
      <div class="col-12 q-mb-sm text-primary text-h6 text-bold">
        Mode :
        <small class="text-normal text-dark"
          >text (default), number, currency, date, datetime, daterange</small
        >
      </div>
      ...
      <lv-input
        ...
        label="Text"
        type="email"
        v-model="inputModel.string"
      />
      <lv-input
        class="col-sm-4"
        mode="number"
        label="Currency"
        v-model="inputModel.integer"
      />
      <lv-input
        class="col-sm-4"
        mode="currency"
        :decimal="2"
        label="Currency with Decimal"
        v-model="inputModel.decimal"
      >
      ...
      <lv-input
        ...
        mode="datetime"
        label="Date Time"
        v-model="inputModel.date_time"
      />
      <lv-input
        class="col-sm-4"
        mode="daterange"
        label="Date Range"
        v-model="inputModel.date_range"
      />

      <q-separator class="col-12 q-my-md" />

      <div class="col-12 q-mb-sm text-primary text-h6 text-bold">Variant</div>
      ...
      <lv-input
        ...
        v-model="inputModel.string"
        flat
        placeholder="Placeholder & flat mode"
      />
      <lv-input
        class="col-sm-3"
        label="type=password"
      ...
      <lv-input
        ...
        label="With Rules"
        v-model="inputModel.string"
        placeholder="error triggered when value not inputed"
        :rules="$Handler.rules('required')"
      />
      <lv-input
        class="col-sm-4"
        label="Hint Slot"
        v-model="inputModel.string"
        @input="(e) => onInput('@input', e)"
      >
      ...
      <lv-input
        ...
        v-model="inputModel.string"
        error
        error-message="Set Error manualy"
      />
      <lv-input
        class="col-sm-4"
        label="Readonly"
      ...
      <lv-input
        class="col-sm-4"
        label="Rules with props 'required'"
        v-model="inputModel.string"
        required
        hint="when using required, rules required auto set"
        clearable
      />
    ...
    <lv-container id="select" class="col-12" height="1050px">
      ...
      <div class="col-12 q-mb-sm text-primary text-h6 text-bold">
        Mode :
        <small class="text-normal text-dark"
          >options (static / dynamic), url (ajax)
        </small>
      </div>
      ...
      <lv-select
        class="col-sm-12"
        top-label
        label="Static with top-label"
        v-model="selectModel.select"
        :options="select.countries"
      />
      <lv-select
        class="col-sm-4"
        label="Static Autocomplete"
        v-model="selectModel.select"
        :options="select.countries"
        searchable
        clearable
      ...
      <lv-select
        class="col-sm-4"
        label="Ajax"
        v-model="selectModel.select_ajax"
        clearable
        url="users"
      ...
      <lv-select
        class="col-sm-4"
        label="Ajax with auto fetch default"
        v-model="selectModel.select_ajax_auto"
        url="users"
        :search-by="['name', 'email']"
      ...
      <lv-select
        class="col-sm-6"
        label="Ajax with manual set default"
        v-model="selectModel.select_ajax_default"
        url="users"
        :search-by="['name', 'email']"
      ...
      <lv-select
        ...
        :default-data="defaultData"
        :option-label="
          (opt) => (opt ? `${opt.name} || <small>${opt.email}</small>` : '...')
        "
      ...
      <lv-select
        class="col-sm-4"
        label="Ajax with Additional query params "
        v-model="selectModel.select_ajax_default"
        url="users"
        :search-by="['name', 'email']"
        :default-data="defaultData"
    ...
    <lv-container id="toggle" class="col-12" height="440px">
      ...
      <lv-toggle
        class="col-sm-3"
        flat
        label="Flat witch icon true / false"
        checked-icon="check"
    ...
    <lv-container id="container" class="col-12" height="800px">
      ...
      <p class="text-red-8 text-bold">
        * For this component, event <i class="text-dark">@input</i> not
        available, use <i class="text-dark">@update</i>, that similar like
        <i class="text-dark">@input</i>
      </p>
      ...
      <lv-textarea
        ...
        top-label
        rows="1"
        v-model="inputModel.textarea2"
        @update="(e) => onInput('@update', e)"
      />
      <lv-textarea
        class="col-sm-6"
        label="Custom Rows"
    ...
    <lv-container id="container" class="col-12" height="200px">
      ...
      <textarea class="col-12 codeplace" rows="3">
<!--Card for display default content-->
    ...
    <lv-container id="common" class="col-12" height="1500px">
      ...
      <div class="col-12">
        <span class="code text-bold">&lt;lv-btn&gt;</span> <br />
      </div>
      <div class="q-gutter-sm q-mb-md">
        ...
        <lv-btn
          label="auto hide label on xs / mobile use : labelVisibility"
          color="teal"
          icon="check"
          labelVisibility
        />
        <lv-btn
          label="redirect using :to"
          color="orange-8"
          :to="{ name: 'home' }"
        />
      ...
      <div class="col-12">
        <span class="code text-bold">&lt;lv-banner&gt;</span> <br />
        <i class="text-indigo-8"
          >* when using <b>dismiss slot</b> , content : col-11 & dismiss slot :
          col-1</i
        >
      </div>

      <lv-banner class="col-12 q-mb-md"> ⚠️ Banner default </lv-banner>

      <lv-banner class="col-12 q-mb-md" solid>
        ⚠️ Banner solid default
      </lv-banner>

      <lv-banner class="col-12 q-mb-md" dense> ❤ Banner dense </lv-banner>
      ...
      <div class="col-12">
        <span class="code text-bold">&lt;lv-displayer&gt;</span> <br />
      ...
      <div class="col-4 border q-pa-sm">
        ...
        <lv-displayer
          class="col-12"
          :data="{ id: 1, name: 'John', birthdate: '2022-01-10' }"
        />
      </div>
      ...
      <div class="col-12">
        <span class="code text-bold">&lt;lv-view-item&gt;</span> <br />
        <i class="text-indigo-8"
          >* can auto formater value display using optimize-display, the display
          using &lt;lv-displayer&gt;</i
        >
      </div>
      <p># Basic</p>
...
export default defineComponent({
  ...
  setup() {
    ...
    const defaultData = {
      id: 9,
      name: "Prof. Dayton Johnston",
      email: "wweissnat@iqbal.org",
    };
