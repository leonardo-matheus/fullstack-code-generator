  ...
  <div class="row">
    ...
    <div class="col-12 q-pa-md">
      <lv-banner>
        <span class="text-h5 text-bold"
          >Bem vindo ao <span class="text-dark">Lavarel</span> module
          Generator.</span
        >
        <p class="text-dark">
          The data you see here is taken directly from local data which is
          located at :
          <code class="code-block">\generator\scopes</code>
        </p>
        <p>
          Para gerar seus modulos utilize este comando : <br />
          Gerar todos os escopos :<code class="code-block text-dark"
            >$ php artisan Lavarel-generate</code
          >
          <br />
          Gerar escopo específicos :
          <code class="code-block text-dark"
            >$ php artisan Lavarel-generate --s=scope_name
          </code>
          <br />
          <br />
          <span class="text-bold text-red-9"
            >* O controller é somente para a API. Você precisa criar seu
            próprio.</span
          >
        </p>
        <p>
          <b>Para faz Deploy da Aplicação : </b><br /><br />
          1. Execute
          <code class="code-block text-dark">$ quasar build</code> dentro da
          pasta /ui, e aguarde finalizar.<br />
          2. Clique
          <lv-btn
            flat
            size="sm"
            label="Generate Route UI"
            @click="downloadRoute"
          />
          , copie e cole o arquivo dentro da "/routes" , tenha certeza que o
          nome é "ui_route.json"<br />
          3. Execute
          <code class="code-block text-dark">$ php artisan Lavarel-deploy</code>
          na pasta root <br />
        </p>
      </lv-banner>
    </div>
    ...
    <div class="row col-12 q-pa-sm">
      <div
        ...
        v-for="(scope, i) in list"
        :key="`md-${i}`"
      >
        <q-card
          class="animated zoomIn bg-primary text-center link"
        ...
        >
          ...
          <q-card-section class="q-pa-sm bg-white">
            ...
            <div class="text-subtitle2 text-grey-7">
              {{ scope.modules ? scope.modules.length : 0 }} Modulos
            </div>
          </q-card-section>
        </q-card>
      </div>
    ...
    >
      <q-card v-if="modal.data" style="min-width: 90vw">
        <q-card-section class="row items-center q-py-xs bg-grey-2">
          <div class="">
            <lv-btn
              ...
              icon="delete"
              color="red"
              @click="removeScope(modal.data.id)"
            />
          ...
          <q-tab-panels v-model="tab" animated>
              ...
              >
                <template v-slot:col-act="props">
                  <lv-btn
                    ...
                    icon="delete"
                    color="red"
                    @click="removeModule(props.row, props.index)"
                  />
                  <lv-btn
                    icon="table_chart"
                    color="indigo"
                    @click="toStructure(props.row)"
                    class="q-ml-sm"
                  >
                    <q-tooltip>Ver Tabela</q-tooltip>
                  </lv-btn>
                </template>
            ...
            <q-tab-panel name="structure" class="row q-pa-none">
              ...
              >
                <template v-slot:col-act="props">
                  <lv-btn
                    v-if="props.row.type !== 'bigIncrements'"
                    soft
                    icon="delete"
                    color="red"
                    @click="removeColumn(props.row, props.index)"
                  />
                </template>
                ...
                <template v-slot:col-name="props">
                  <lv-input
                    ...
                    :error="
                      errorColumn.index === props.index && errorColumn.message
                        ? true
                        : false
                    "
                    :error-message="`${
                ...
                <template v-slot:col-type="props">
                  <div class="row" style="width: 150px">
                    <lv-select
                      ...
                      :options="select.type"
                      @input="(val) => onInputColumn('type', props.index, val)"
                    />
                ...
                <template v-slot:col-specs="props">
                  <div class="row" v-if="props.row.type !== 'morph'">
                    <template v-if="props.row.type !== 'bigIncrements'">
                      <q-select
                        v-if="props.row.type === 'enum'"
                        label="Enum List"
                        class="col-12"
                        v-model="props.row.enum_list"
                        filled
                        dense
                        use-input
                        use-chips
                      ...
                      <lv-input
                        col="6"
                        v-if="
                          props.row.type === 'integer' ||
                          props.row.type === 'double'
                        "
                        mode="number"
                        label="Length"
                      ...
                      <lv-input
                        col="6"
                        v-if="props.row.type === 'double'"
                        mode="number"
                        label="Length 2"
                        v-model="props.row.length2"
                      />
                    </template>
                  </div>
                  <span v-else class="text-indigo-9">
                    Coluna vai ser gerada automaticamente <br />
                    {{ props.row.name }}_type & {{ props.row.name }}_id
                  </span>
                </template>
                ...
                <template v-slot:col-default="props">
                  <template
                    v-if="
                      props.row.type !== 'bigIncrements' &&
                      props.row.type !== 'morph'
                    "
                  >
                    <lv-toggle
                    ...
                    <lv-input
                      v-else
                      :mode="getInputType(props.row.type)"
                      v-model="props.row.default"
                      :decimal="props.row.length2 ? props.row.length2 : 0"
                    />
                  </template>
                </template>
                ...
                <template v-slot:col-belongsTo="props">
                  <template v-if="props.row.type !== 'morph'">
                    <lv-displayer
                      dense
                      v-if="props.row.belongsTo"
                      :data="[props.row.belongsTo]"
                      @click="
                        openModalRelation(
                          'belongsTo',
                          props.index,
                ...
                <template v-slot:col-hasMany="props">
                  <template v-if="props.row.type !== 'morph'">
                    <lv-displayer
                      dense
                      v-if="props.row.hasMany"
                      :data="[props.row.hasMany]"
                      @click="
                        openModalRelation(
                          'hasMany',
                          props.index,
        ...
        <q-card-section class="col-12 q-pa-lg text-center" v-else>
          <span class="text-h5 text-bold"
            >Clique em "Add Module" para refinar o escopo
          </span>
          <p class="q-pt-md">
            Você pode alterar o nome do escopo, no canto superior esquerdo<br />
            Para previnir erros, utilize "snake_case" no nome do escopo.
          </p>
        </q-card-section>
      </q-card>
    </q-dialog>
    ...
    <q-dialog
      v-model="modalRelation.show"
      @hide="modalRelation.show = false"
      transition-show="jump-up"
      transition-hide="jump-down"
    >
      <q-card v-if="modalRelation.data">
        <q-card-section class="row items-center q-py-xs bg-grey-2">
          <div class="text-h6">
            {{ modalRelation.type }} : {{ modalRelation.data.name }}
          </div>
          <q-space />
          <q-btn icon="close" flat round dense v-close-popup class="q-ml-sm" />
        </q-card-section>
        ...
        <q-card-section class="col-12 q-gutten-md">
          <q-form @submit="saveRelation()" class="col-12 row animated fadeIn">
            <lv-input
              class="q-mb-sm"
              label="Name*"
              v-model="modalRelation.data.name"
              placeholder="Use PascalCase"
              required
            />
            ...
            <lv-input
              class="q-mb-sm"
              v-if="
                modalRelation.type === 'belongsTo' ||
                modalRelation.type === 'hasMany'
              "
              label="Model*"
              v-model="modalRelation.data.model"
              placeholder="Use PascalCase"
              required
            />
            ...
            <lv-input
              ...
              readonly
              label="Foreign Key*"
              v-model="modalRelation.data.foreign_key"
              hint="Auto preenchimento"
            />

            <lv-input
              class="q-mb-sm"
              v-if="modalRelation.type === 'belongsTo'"
              label="Owner Key"
              v-model="modalRelation.data.owner_key"
            />
            <lv-input
              class="q-mb-sm"
              v-if="modalRelation.type === 'hasMany'"
              label="Local Key"
              v-model="modalRelation.data.local_key"
            />
...
export default defineComponent({
  ...
  setup() {
    ...
    function removeModule(data, i) {
      const message = `Are you sure want to delete module "${
        data ? data.name : ""
      }"?`;
      Dialog.create({
        title: "Delete Module",
    ...
    function removeColumn(data, i) {
      const message = `Are you sure want to delete column "${
        data ? data.name : ""
      }"?`;
      Dialog.create({
        title: "Delete Column",
