          ...
          <q-card class="shadow">
            <q-card-section class="row">
              <lv-select
                ...
                label="Group"
                col="12"
                v-model="dataModel.role_group_id"
                url="role-groups"
                limit="20"
                :default-data="dataModel.role_group"
              />
              <lv-input
                top-label
                label="name"
                ...
                :rules="$Handler.rules(dataModel.name, 'required')"
              />
              <lv-input
                top-label
                label="slug"
                ...
                :rules="$Handler.rules(dataModel.slug, 'required')"
              />
            </q-card-section>
          </q-card>
        </div>
        ...
        <div class="col-12 col-sm-8 q-px-sm">
            ...
            <template v-slot:header="props">
              <q-tr :props="props">
                <q-th
                  ...
                  class="bg-dark"
                >
                  <span class="text-bold text-h6 text-white"
                    >{{ col.label }} ({{ totalAllPermissions(true) }})</span
                  >
                </q-th>
              </q-tr>
            </template>
                ...
                >
                  <div>
                    <q-list bordered separator class="bg-grey-1">
                      <q-item dense>
                        <q-item-section>
                          <div class="row">
                            <lv-btn
                              @click="
                                batchPermission(props.row.permissions, true)
                              "
                              class="col-6"
                              size="xs"
                              icon="fact_check"
                            ...
                            <lv-btn
                              @click="
                                batchPermission(props.row.permissions, false)
                              "
                              v-if="
                                totalPermissions(props.row.permissions, true) >
                                0
                              "
                              class="col-6"
                              size="xs"
                      ...
                      <q-item
                        dense
                        v-for="(row, i) in props.row.permissions"
                        :key="i + 'tgl' + props.row.name"
                      >
                        ...
                        <q-item-section
                          :class="row.allow ? 'text-green-9' : 'text-red-9'"
                          >{{ row.name }}</q-item-section
                        >
                      </q-item>
                    </q-list>
                  </div>
          ...
          <div v-else class="text-center">
            <lv-banner color="indigo">
              ...
              <div class="text-bold">
                For first time creating Role, you cannot define the permissions
              </div>
              <div>
                if you want to define the permissions, after saving this data
                you can define by edit the data.
              </div>
            </lv-banner>
          </div>
        </div>
