    ...
    <q-form @submit="submit" v-if="!loading" class="col-12 row animated fadeIn">
      ...
      <div class="ph-1 col-12 q-pa-md">
        <lv-banner color="blue" dense>
          This form is auto save, you can change any setting without click
          button save.
        </lv-banner>
      </div>
          ...
          <q-tab-panels v-model="tab" animated>
            <template v-for="(stg, i) in list" :key="i + 'span'">
              <q-tab-panel :name="stg.name" class="row">
                <div
                  class="col-12 q-px-lg q-py-sm q-mb-lg bg-yellow-2 round quote"
                >
                  <span class="">{{ stg.comment ?? "" }}</span>
                </div>

                <template
                  v-for="(val, attr) in stg.list"
                  :key="i + 'sitem' + attr"
                >
                  <lv-input
                    class="q-mb-md"
                    v-if="val.raw.type === 'string' || val.raw.type === 'NULL'"
                    @input="
                      updateSetting(val.raw.id, val.value, stg.name, attr)
                    "
                    col="12"
                    :label="$Helper.toLabel(attr)"
                    v-model="val.value"
                  />
                  <lv-toggle
                    class="q-mb-md"
                    v-if="val.raw.type === 'boolean'"
                    @input="
                      updateSetting(val.raw.id, val.value, stg.name, attr)
                    "
                    col="6"
                  ...
                  <lv-textarea
                    class="q-mb-md"
                    v-if="val.raw.type === 'array'"
                    @input="
                      updateSetting(val.raw.id, val.raw_value, stg.name, attr)
                    "
                    col="12"
                    :label="$Helper.toLabel(attr)"
                    v-model="val.raw_value"
                  />
                </template>
              </q-tab-panel>
            </template>
