...
export default class Api {
      ...
      if (payload) {
        for (const key in payload) {
          if (payload[key]) {
            ...
            else if (valueFormated instanceof File)
              valueFormated = valueFormated;
            else if (typeof valueFormated === "object")
  ...
  validateResponseData(reCompile) {
    ...
    if (reCompile === null) {
      ...
      Helper.showAlert(
        "😅 Not found!",
        msg ? msg : "Page or host target (404) Not found"
      );
      return false;
  ...
  failedConnect(reCompile) {
    Dialog.create({
      ...
      message: `Can't access url : <br> <small class="text-bold">${
        reCompile ? reCompile.url : ""
      }</small> <br> please try again!`,
      class: "vl-dialog",
      ok: "Close",
      html: true,
      position: "top",
  ...
  getCache(full, defaultVaule = "") {
    ...
    if (cache) {
      if (full.config !== undefined) {
        var checkCacheLDB = Helper.checkLdb(
          full.config.method + ":" + full.config.url
        );
