...
export const Handler = {
  ...
  topMenuCallEvent(params, index) {
    if (!params) return;
    if (params[index].event !== null && params[index].event !== undefined)
  ...
  table(columns = [], cacheNameVisColumn = null, limit = 16) {
    ...
    for (const col of columns) {
      ...
      col.searchable = push;
      if (col.name !== "action" && col.field) {
        // default only set 2 column to global search
      ...
      col.searching = {
        ...
        value2: null,
        operator:
          col.searching && col.searching.operator
            ? col.searching.operator
            : "like",
        operator2:
          col.searching && col.searching.operator2
            ? col.searching.operator2
            : null,
      };
    ...
    */
    ...
    // minumum & maximum character
      ...
      if (mxVal.length === 2) {
        ...
        res = [
          (val) =>
            (!!val && val.length >= minVal) || _msg
              ? _msg
              : `Min ${minVal} characters`,
          (val) =>
            (!!val && val.length <= maxVal) || _msg
              ? _msg
              : `Max ${maxVal} characters`,
        ];
      }
    }
    // minumum & maximum number
      ...
      if (mxVal.length === 2) {
        ...
        res = [
          (val) =>
            (!!val && val >= minVal) || _msg ? _msg : `Min value is ${minVal}`,
          (val) =>
            (!!val && val <= maxVal) || _msg ? _msg : `Max value is ${maxVal}`,
        ];
      }
    }
    // required
