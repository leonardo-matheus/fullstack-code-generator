...
export const Helper = {
  ...
  showAlert(
    ...
    if (title === "try") {
      judul = "Opps!";
      pesan =
        "Terjadi kesalahan saat menghubungkan ke server, harap coba lagi atau tekan Refresh!";
    }
  ...
  successAlertBody(title, msg = null, className = "text-center") {
    ...
    var judul =
      '<div class="text-center q-dialog__title text-primary capital text-bold">' +
      title +
      "</div>";
    var pesan =
      '<div class="' + className + ' animated fadeIn">' + msg + "</div>";
    ...
    if (payload) {
      for (const key in payload) {
        if (payload[key]) {
          ...
          if (typeof valueFormated === "object")
            valueFormated = JSON.stringify(valueFormated);
  ...
  toDate(tanggal, format = "YYYY-MM-DD") {
    // full date format sql : YYYY-MM-DD HH:MM / beauty DD MMM YYYY
  ...
  toLabel(text, separator = "_") {
    ...
    if (text) {
      if (separator === "-")
        res = text.replace(/(?:-| |\b)(\w)/g, function ($1) {
          return $1.toUpperCase().replace(separator, " ");
        });
      else
        res = text.replace(/(?:_| |\b)(\w)/g, function ($1) {
          return $1.toUpperCase().replace(separator, " ");
        });
    }
    return res;
